...
def create_studentDatabase(studentRecords):
    ...
    size = 7   # Initializing size as given in the question
    
    H = create_hashtable(size)   # Initializing hashtable

    # Iterating over the records list to populate the hashtable
...
def perform_Operations(H, operationFile):
    ...
    # Initializing size, operations list and collision path
    size = len(H[0])
    operations = []
    collision_path = {}
    ...
    for line in lines:
        operations.append(line.strip())   # Processing to get the desired list

    count = 1   # OpNumber
    for op in operations:
        ToDo = op.split(' ')
        
        # In case of 'Find' call get function
        if ToDo[0].upper() == 'FIND':

            ToDo.append(-1)
            key = ToDo[1]
            record = get(H, key, size, collision_path, count)
            if ToDo[2] != -1:
                print(record[ToDo[2]])
            else:
                print(record)

        # In case of 'Update' call Update function
        ...
        # In case of 'Delete' call Update function
...
def main(filename):
    ...
    with open(filename) as file:
        for line in file:
            StR.append(line.strip().split(','))   # Processing and appending separate records

    StR = [x for x in StR if x != ['']]   # Removing empty lists

    for i in range(1,len(StR)):
        
        # Processing to get list of dictionaries
